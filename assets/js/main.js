...
async function handleCalcInvestment() {
  ...
  let cap = parseFloat(
    valueInvested[investmentType].value.replace(/\./g, "").replace(",", ".")
  );
  ...
  valueInvested[investmentType].grossIncome =
...
function handleCalcSavings() {
  ...
  // Arredonda o valor para duas casa decimanis depois da virgula e injeta no objeto como um Number;
...
function handleShowInvestedTotals() {
  amountInvestment.innerHTML = formmaterCurrencyValue(
    valueInvested[investmentType].amount
  );
  totalsInvested.innerHTML = formmaterCurrencyValue(
    valueInvested[investmentType].value
  );
  totalsGrossIncome.innerHTML = formmaterCurrencyValue(
    valueInvested[investmentType].grossIncome
  );
  savingsInvested.innerHTML = formmaterCurrencyValue(
    valueInvested[investmentType].savings
  );
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (label) =>
              `R$ ${formmaterCurrencyValue(label.formattedValue)}`,
          },
        },
      },
    },
